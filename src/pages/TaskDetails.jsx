...
const TaskDetails = () => {
    ...
    const handleAddComment = async () => {
        ...
        try {
            ...
            const response = await fetch(buildApiUrl(`/api/v1/tasks/single/${taskId}/comments`), {
                method: "POST",
                headers: {
    ...
    useEffect(() => {
        async function loadComments() {
            if (!taskId || !isLoaded || !isSignedIn) return;
        ...
        if (taskId && task && isLoaded && isSignedIn) {
            loadComments();
    ...
    }, [taskId, task, isLoaded, isSignedIn, getToken]);
                            ...
                            <div className="flex flex-col gap-4 mb-6 mr-2">
                                {comments.map((comment) => (
                                    <div
                                        ...
                                        className={`sm:max-w-4/5 dark:bg-gradient-to-br dark:from-zinc-800 dark:to-zinc-900 border border-gray-300 dark:border-zinc-700 p-3 rounded-md ${comment.user?.id === currentUserId ? "ml-auto" : "mr-auto"}`}
                                    >
                                        <div className="flex items-center gap-2 mb-1 text-sm text-gray-500 dark:text-zinc-400">
                                            <img src={comment.user?.image || comment.user?.imageUrl} alt="avatar" className="size-5 rounded-full" />
                ...
                <div className="p-5 rounded-md bg-white dark:bg-zinc-900 border border-gray-300 dark:border-zinc-800 ">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm text-gray-700 dark:text-zinc-300">
                        <div className="flex items-center gap-2">
                            <img src={task.assignee?.image || task.assignee?.imageUrl} className="size-5 rounded-full" alt="avatar" />
